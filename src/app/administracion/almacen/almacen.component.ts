...
import { EstadosAlmacenFitl } from './almacen.class';
...
export class AlmacenComponent implements OnInit {
  ...
  c_filter_table = EstadosAlmacenFitl;
  c_filter: any;
  ...
  nuevo(){
    this.event_table_almacenes({tipo: 1})
  }

  // ?----------------------------Almacenes---------------------------
  event_table_almacenes(event: any): void {
      switch (event.tipo) {
          case 1: // * Nuevo
  ...
  event_form_almacenes(event): void {
      switch (event.tipo) {
          case 1: // * Cerró
